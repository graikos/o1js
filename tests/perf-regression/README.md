...
Both use a per-backend JSON baseline file:

- **Wasm**: [perf-regression-wasm.json](./perf-regression-wasm.json)
- **Native**: [perf-regression-native.json](./perf-regression-native.json)

The correct file is automatically selected by CI based on the `O1JS_BACKEND`
environment variable. Baselines should only be dumped by triggering the CI
workflow (see [CI Integration](#ci-integration)).
...
### Dump all examples (local, wasm)

```bash
npm run regression:dump-perf
```

### Check all examples (local, wasm)

```bash
npm run regression:check-perf
```

### Dump all examples (local, native)

```bash
npm run regression:dump-perf:native
```

### Check all examples (local, native)

```bash
npm run regression:check-perf:native
```

## CI Integration

- **PR checks**: Automatically run in `--check` mode for both wasm and native
  backends. CI compares results against the committed baseline for each backend.
...
| **CI (PR)**          | `--check`            | Ensures performance consistency against the repo's baseline (per backend).                    |
| **CI (Manual Dump)** | `--dump`             | Refreshes the baseline via GitHub runners and commits updates (per backend).                  |
